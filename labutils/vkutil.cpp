...
namespace labutils
{
	ShaderModule load_shader_module( VulkanContext const& aContext, char const* aSpirvPath )
	{
		...
		if (std::FILE* fin = std::fopen(aSpirvPath, "rb"))
		{
			...
			//Create, next create the VkShaderModule with vkCreateShaderModule and VkShaderModuleCreateInfo.
	...
	VkCommandBuffer alloc_command_buffer( VulkanContext const& aContext, VkCommandPool aCmdPool )
	{
		...
		//We can use the function to allocate multiple command buffers at once
		...
		if (cbuff == VK_NULL_HANDLE)
		{
			std::fprintf(stderr, "Error, empty-----------------------------------------------------------------------------------------\n");
		}
		return cbuff;
	}
	...
	{
		//Create fence object with fence info
	...
	Sampler create_anisotrpic_sampler(VulkanContext const& aContext)
	{
		...
		if (deviceFeatures.samplerAnisotropy == VK_TRUE) {
			std::fprintf(stderr, "Max anisotropy limit (%f)\n", deviceProperties.limits.maxSamplerAnisotropy);
		}
		else {
			std::fprintf(stderr, "No anisotropy limit (%d)\n");
		}

...
#		endif // ~ debug builds
		...
		ret.window = glfwCreateWindow(1920, 1080, "CW 1", nullptr, nullptr);
		if (!ret.window)
		{
			char const* errMsg = nullptr;
			glfwGetError(&errMsg);
	...
	SwapChanges recreate_swapchain(VulkanWindow& aWindow)
	{
		...
		// Destroy old objects (except for the old swap chain) 9
		// We keep the old swap chain object around, such that we can pass it 10
		// to vkCreateSwapchainKHR() via the oldSwapchain member of 11
		// VkSwapchainCreateInfoKHR. 12
		...
		{
			// Put pack the old swap chain handle into the VulkanWindow; this 35
			// ensures that the old swap chain is destroyed when this error 36
			// branch occurs. 37
		...
		if (oldExtent.width != aWindow.swapchainExtent.width || oldExtent.height != aWindow.swapchainExtent.height) ret.changedSize = true;
...
namespace
{
	std::vector<VkSurfaceFormatKHR> get_surface_formats(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		{
			std::fprintf(stderr, "Error: unable to get phyiscal device surface formats count\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfaceFormatsKHR() returned error %s\n", lut::to_string(res).c_str());
			return {};
		}
		...
		{
			std::fprintf(stderr, "Error: unable to get phyiscal device surfaceFormats list\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfaceFormatsKHR() returned error %s\n", lut::to_string(res).c_str());
			return {};
		}

		//std::fprintf(stderr, "size of format %d\n", surfaceFormats.size());
	...
	std::unordered_set<VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		{
			std::fprintf(stderr, "Error: unable to get phyiscal surface present modes count\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfacePresentModesKHR() returned error %s\n", lut::to_string(res).c_str());
			return {};
		}
		...
		{
			std::fprintf(stderr, "Error: unable to get phyiscal device present modes list\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfacePresentModesKHR() returned error %s\n", lut::to_string(res).c_str());
			return {};
		}

		std::unordered_set<VkPresentModeKHR> presentModesSet(numPresentModes);

		for (auto presentMode : presentModes)
			presentModesSet.insert(presentMode);
	...
	std::tuple<VkSwapchainKHR, VkFormat, VkExtent2D> create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
	{
		...
		case VK_FORMAT_B8G8R8A8_UNORM:
			formatName = "VK_FORMAT_B8G8R8A8_UNORM";
			break;
		case VK_FORMAT_R8G8B8A8_UNORM:
			formatName = "VK_FORMAT_R8G8B8A8_UNORM";
			break;
		case VK_FORMAT_B8G8R8A8_SRGB:
			formatName = "VK_FORMAT_B8G8R8A8_SRGB";
			break;
			// Add other cases for different formats if needed.
		default:
			formatName = "Unknown Format";
			break;
		}
		std::fprintf(stderr, "Swapchain iamge formats: %s\n", formatName);


		//TODO: pick appropriate VkPresentModeKHR
		VkPresentModeKHR presentMode = VK_PRESENT_MODE_FIFO_KHR; // FIXME
		...
		std::uint32_t imageCount = 2; // FIXME
	...
	void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages)
	{
		...
		{
			std::fprintf(stderr, "Error: unable to get swapchain_images count\n");
			std::fprintf(stderr, "vkGetSwapchainImagesKHR() returned error %s\n", lut::to_string(res).c_str());
			return;
		}

		std::fprintf(stderr, "Num of swapchain images: %d\n", numImages);

		std::vector<VkImage> temp(numImages);

		if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &numImages, temp.data()); VK_SUCCESS != res)
		{
			std::fprintf(stderr, "Error: unable to get swapchain_images list\n");
			std::fprintf(stderr, "vkGetSwapchainImagesKHR() returned error %s\n", lut::to_string(res).c_str());
			return;
		}
...
namespace
{
	float score_device(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		//TODO:    commands
		// Check that the device supports the VK KHR swapchain extension 1
		...
		// Ensure there is a queue family that can present to the given surface 10
